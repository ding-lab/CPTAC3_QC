...
##################################### 05/13/19 (Finalization) #########################################
######################## WXS CNV quality control (QC) pipeline (GATK4 beta output) ####################
#######################################################################################################
...
## All arguments should be defined in WXS_CNV_QC_input.R by the users.
...
if (args_count < 3) {
  stop("At least three argument must be provided to the CNV QC pipeline.", call.=FALSE)
    ...
    CNV_QC(path_input, path_output, input_format, mean_l_bound, mean_u_bound, good_cutoff, bad_cutoff)
    ...
    CNV_QC(path_input, path_output, input_format, mean_l_bound, mean_u_bound, good_cutoff, bad_cutoff, clinic_file)
} else if (args_count == 9) {
    print("All the required and optional information has been provided by the user.")
    CNV_QC(path_input, path_output, input_format, mean_l_bound, mean_u_bound, good_cutoff, bad_cutoff, clinic_file, sample_type_file)
} else if (args_count == 10) {
    print("All required and optional information has been provided by the user including the Y cutoff.")
    CNV_QC(path_input, path_output, input_format, mean_l_bound, mean_u_bound, good_cutoff, bad_cutoff, clinic_file, sample_type_file, Y_cutoff)
}
